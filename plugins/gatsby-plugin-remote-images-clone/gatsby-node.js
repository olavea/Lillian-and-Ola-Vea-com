...
async function createImageNodes(urls, node, options, reporter, silent) {
  ...
  ).filter((fileNode) => !!fileNode); // Store the 🗺️ mapping between the current node and the newly created File node
  ...
  if (fileNodes.length) {
    ...
    // of this 🗺️ mapping. The 🔑 keys are based on the ID of the 💰 parent node (of
